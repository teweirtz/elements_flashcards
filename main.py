...
db = PostgresqlDatabase('elements', user='', password='',host='localhost', port=5432)
...
def review():
    correct = 0
    i = int(input("How many elements would you like to study? "))
    total = i
    for card in Elements.select():
        if i > 0:
            i -= 1
            if input(f"{card.name}\n") == card.abbreviation:
                print(f"The answer is {card.abbreviation}")
                correct += 1
